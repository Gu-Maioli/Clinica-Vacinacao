...
export default function Tabela(props) {
  const editCompra = (id, nomePaciente, descricao, dataInicio, paciente_id) => {
    props.editFunc({
      id: id,
      descricao: descricao,
      dataInicio: arrumaDataToSave(dataInicio.split(".")[0]),
      paciente_id: paciente_id,
    });
  };

  const deleteCompra = (id) => {
    Axios.delete(`http://localhost:3001/deleteAtendimento/${id}`).then(() => {
      props.setLista(
  ...
  return (
    <>
      <tr key={props.id}>
        ...
        <td className="">{arrumaData(props.dataInicio)}</td>
